...
public class RoleController {
	...
	public ResponseEntity<Map<String, Object>> list(@RequestParam(required = false) Long id,
		...
		try {
			if (id != null) {
				RoleDto one = service.getById(id);
				resp.put("result", one);
			} else {
				List<RoleDto> data = service.list(companyId, q, page, size);
	...
	public ResponseEntity<Map<String, Object>> update(@Valid @RequestBody RoleDto dto) {
		...
		try {
			...
			RoleDto updated = service.update(dto.getRoleId(), dto);
	...
	public ResponseEntity<Map<String, Object>> delete(@RequestBody Map<String, Object> body) {
		Map<String, Object> resp = new LinkedHashMap<>();
		try {
			Long id = asLong(body.get("roleId"));
			if (id == null)
				throw new IllegalArgumentException("roleId is required");
