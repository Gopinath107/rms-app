...
public class ResumeStorageService {
		...
		if ("aws".equalsIgnoreCase(this.activeStorageProvider)) {
			if (s3 == null) {
				throw new IllegalStateException(
						"storage.provider is 'aws' but S3Client is not configured. Check AWS credentials/config.");
			}
			this.isAws = true;
			this.localPath = null;
			this.s3 = s3;
			...
			this.isAws = false;
			this.localPath = Paths.get(this.activeStorageProvider);
			this.s3 = null;
			this.bucket = "";
		}
	}

	public StoredObject upload(long employeeId, String originalFileName, String declaredMime, InputStream data,
		...
		String key = "resume/%d-%s-%s".formatted(employeeId, uuid8, safeName);
	...
	public ResumeResource load(EmployeeDocument doc) throws Exception {
		if ("aws".equalsIgnoreCase(doc.getStorageProvider())) {
			if (this.s3 == null) {
				throw new IllegalStateException("AWS S3 client is not configured.");
			}
			GetObjectRequest getRequest = GetObjectRequest.builder().bucket(this.bucket).key(doc.getStorageKey())
					.build();
			...
			return new ResumeResource(resource, doc.getMimeType(), doc.getDocumentName());

		} else {
			try {
				Path localFile = Paths.get(doc.getFilePath());
				Resource resource = new UrlResource(localFile.toUri());

				if (resource.exists() && resource.isReadable()) {
					return new ResumeResource(resource, doc.getMimeType(), doc.getDocumentName());
				} else {
					throw new RuntimeException("Could not read local file: " + doc.getFilePath());
				}
			} catch (MalformedURLException e) {
				throw new RuntimeException("Error reading local file: " + doc.getFilePath(), e);
			}
		}
	}
	...
	public StoredObject uploadBytes(long employeeId, String fileName, String mimeType, byte[] bytes) throws Exception {
	    try (var in = new java.io.ByteArrayInputStream(bytes)) {
	        return upload(employeeId, fileName, mimeType, in, bytes.length);
	    }
	}
