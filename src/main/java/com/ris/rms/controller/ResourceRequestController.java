...
public class ResourceRequestController {
	...
	/**
	 * Returns only real (resume-linked) resource requests for a specific demand.
	 * Used by Schedule Interview modal and Demand Details page.
	 * GET /api/resource-requests/by-demand/{demandId}
	 */
	@GetMapping("/by-demand/{demandId}")
	public ResponseEntity<Map<String, Object>> getByDemand(@PathVariable Long demandId) {
		Map<String, Object> resp = new LinkedHashMap<>();
		try {
			List<ResourceRequestDto> data = service.getByDemand(demandId);
			resp.put("result", data);
			resp.put("success", true);
			resp.put("errors", List.of());
			resp.put("errorCount", 0);
