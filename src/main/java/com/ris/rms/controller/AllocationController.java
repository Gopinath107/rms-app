...
public class AllocationController {
	...
	public ResponseEntity<Map<String, Object>> list(@RequestParam(required = false) Long id,
			...
			@RequestParam(required = false) Long employeeId, @RequestParam(required = false) String status,
			@RequestParam(required = false) Boolean billable, @RequestParam(required = false) Integer page,
		...
		try {
			if (id != null) {
				...
				resp.put("result", service.list(companyId, projectId, employeeId, status, billable, page, size));
			}
			resp.put("success", true);
			resp.put("errors", List.of());
			resp.put("errorCount", 0);
